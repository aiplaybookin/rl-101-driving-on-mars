...
# --- PYQT ---
# Ensure Qt can find the platform plugin (e.g. libqcocoa.dylib) when
# running inside conda/venv. We set `QT_QPA_PLATFORM_PLUGIN_PATH` to the
# directory containing plugins so GUI starts reliably.
import sys
import os as _os
def _fix_qt_plugin_path():
    candidates = [
        _os.path.join(sys.prefix, 'plugins'),
        _os.path.join(sys.prefix, f'lib/python{sys.version_info.major}.{sys.version_info.minor}/site-packages/PyQt6/Qt6/plugins/platforms'),
    ]
    for p in candidates:
        try:
            if _os.path.isdir(p):
